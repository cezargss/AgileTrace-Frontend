...
export default {
  ...
  methods: {
    ...
    async update() {
      ...
      try {
        await this.$http.put(
          `api/v1/projects/${this.project.id}/releases/${this.projectRelease.id}`,
          this.formData
        );
        this.$notify(
  ...
  mounted() {
    this.formData = {
      ...this.projectRelease,
      releaseDate: this.projectRelease.releaseDate
        ? new Date(this.projectRelease.releaseDate)
        : null,
    };
