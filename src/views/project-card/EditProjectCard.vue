  ...
  <v-dialog v-model="active" inset persistent max-width="1200">
    <v-card rounded="lg">
      <v-card-title class="d-flex justify-space-between align-center">
        <div class="text-h5 text-medium-emphasis ps-2">Editar Atividade</div>
        <div>
          <v-btn
...
export default {
  ...
  methods: {
    ...
    async update() {
      ...
      try {
        ...
        await this.$http.put(
          `api/v1/projects/${this.project.id}/activities/${this.projectCard.id}`,
          this.formData
        );
        this.$notify(
  ...
  async mounted() {
    ...
    this.formData = {
      ...this.projectCard,
      estimatedDate: this.projectCard.estimatedDate
        ? new Date(this.projectCard.estimatedDate)
        : null,
    };
    this.listUsers();
    this.listCardTypes();
